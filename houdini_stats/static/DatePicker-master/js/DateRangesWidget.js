(function($) {
	...
	var db = {
		...
		aggregations : {
			...
			'daily' : {
				title : "Daily",
				presets : ['custom', 'today', 'yesterday', 'lastweeks', 'lastmonths', 'lastyears'],
				selected: ""
			},
			'weekly' : {
				title : "Weekly",
			...
			'whole' : {
				title : "Whole period",
				presets : ['custom', 'lastweeks', 'lastmonths', 'lastyears']
			}
		},
	...
	var methods = {
		...
		init : function(options) {
			...
			$selected_aggregation = options.aggregation;
	...
	var internal = {
		...
		createElements : function($target) {
			...
			if (!$dropdown) {
				...
				'<div id="datepicker-dropdown">'+
					...
					'<div class="date-ranges-form">'+
						...
						'<div style="display: none;" >'+
							...
							'</select>'+
						'</div>'+
						'<div style="display: none;" class="comparison-daterange">'+
                ...
                // Set proper date for date range
                var from = $custom_start.split(',')
											 .splice(0,2)
											 .join()
											 .replace('.','') // We remove the dot
														      // because Firefox cannot
														      // process it
		        var from_date = new Date(Date.parse(from));
			    var to = $custom_end.split(',')
										 .splice(0,2)
										 .join()
										 .replace('.','')
			    var to_date = new Date(Date.parse(to));
				...
				$datepicker.DatePicker({
					 ...
					 calendars: 3,
					 mode: 'range',
					 current: new Date(to_date.getFullYear(), to_date.getMonth() - 1, 1),
		...
		loadValues : function($target) {
			...
			$aggregation.val($selected_aggregation);
			$aggregation.change();
